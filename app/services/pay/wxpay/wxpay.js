...
class WxPay {
  ...
  static async unifiedOrder(order) {
    let appid = globa.config.miniProgram.appid
    let mch_id = globa.config.pay.wechat.mch_id
    let mchkey = globa.config.pay.wechat.mch_key
    ...
    let notify_url = globa.config.pay.wechat.notify_url
    let openid = 'oKHW15DAIDvVr6ZfFMQ7BIW4RmGo'
    ...
    let paySign = util.paysignjsapifinal(appid, mch_id, prepay_id, nonce_str, timestamp, mchkey)
